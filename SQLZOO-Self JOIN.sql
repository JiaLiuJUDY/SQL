...
SELECT DISTINCT a.company, a.num
  -- Pay attention to the usage of DISTINCT keyword
...
FROM (SELECT a.company, a.num, b.stop
     ...
     (SELECT a.company, a.num, b.stop
      FROM route a JOIN route b ON (a.company=b.company AND a.num=b.num)
      WHERE a.stop=(SELECT id FROM stops WHERE name= 'Sighthill'))E ON (S.stop = E.stop)
      JOIN stops ON(stops.id = S.stop)

-- My SOL [I think is also correct, but didn't pass the test]
SELECT DISTINCT a.num, a.company, f.name, c.num, c.company
FROM (route a JOIN route b ON a.company = b.company AND a.num = b.num) 
      JOIN (route c JOIN route d ON c.company = d.company AND c.num = d.num) ON b.stop = c.stop 
      JOIN stops e ON e.id = a.stop 
      JOIN stops f ON f.id = b.stop 
      JOIN stops g ON g.id = d.stop
WHERE e.name = 'Craiglockhart'  AND g.name = 'Sighthill' 
